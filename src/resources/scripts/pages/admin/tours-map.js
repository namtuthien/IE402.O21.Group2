import { getLocations, getLinesOfTour } from "/fetch.js";
...
require([
  ...
  const routeParams = new RouteParameters({
    ...
    apiKey:
      "AAPK35e6fc8a720d45eb8c319c6b0cf5ddb1CAqeaYpeK-CG-rCiUfYFBhOGIbQq5m5UjPEw4tb9fWciTNaHvj6I6TaRS8KdGPKD",
  ...
  document.getElementById("hideAddDiv").addEventListener("click", function () {
    ...
    choosedLocations.forEach((choosedLocation) => {
  ...
  document
    .getElementById("tour-popup__current-locations")
    .addEventListener("click", function (event) {
      if (event.target.closest(".tour-popup__remove-button")) {
        ...
        if (indexOfRemoved >= 0) {
          ...
          const index = routeParams.stops.features.findIndex((feature) => {
            return (
              feature.attributes.location_name ===
              choosedLocations[indexOfRemoved].attributes.location_name
            );
          });
